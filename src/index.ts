...
}
{
            ...
            case "AWAITING_FILES":
                if (msg.hasMedia) {
                    ...
                    if (["2", "selesai", "done", "lanjut"].includes(lower)) {
                        ...
                        await finalizeOrder(chat, chatId, session);
                    } else {
                        await client.sendMessage(
                            chatId,
    ...
    async function createPrintJob(chat: pkg.Chat, session: UserState) {
        const formData = new FormData();
        const phoneNum = (await chat.getContact()).number;
        formData.append("customer_name", session.customerName || "N/A");
        formData.append("customer_number", phoneNum);
    ...
    async function finalizeOrder(chat: pkg.Chat, chatId: string, session: UserState) {
    ...
    async function generateSummaryAndQr(chat: pkg.Chat, chatId: string, session: UserState) {
        await client.sendMessage(
            chatId,
