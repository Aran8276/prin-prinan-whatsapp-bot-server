...
if (!process.argv.includes("--worker")) {
  ...
  const startWorker = () => {
    const args = [...process.argv.slice(1), "--worker"];
    ...
    child.on("close", (code) => {
      console.warn(
        `[Master] Worker process exited with code ${code}. Restarting in 1s...`,
      );
      setTimeout(startWorker, 1000);
    });

    child.on("error", (err) => {
      console.error(`[Master] Failed to start worker:`, err);
    });
  };

  startWorker();

  // Keep master process alive and handle termination to kill child
  process.on("SIGINT", () => process.exit(0));
  process.on("SIGTERM", () => process.exit(0));

  // Stop execution here for the master process
} else {
  // --- WORKER LOGIC (Your Original Code) ---
  runApp();
}
