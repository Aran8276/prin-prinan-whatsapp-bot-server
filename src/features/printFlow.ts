...
export const calculateFilePrice = async (
  file: FileData,
  chatId: string,
  session: UserState,
): Promise<number> => {
  ...
  const value = file.customPrice ?? 0;
  if (value === 0) {
    await sock.sendMessage(chatId, {
      text: `⚠️ Gagal menghitung harga untuk file: *${file.filename}*. Mohon atur konfigurasi warna dengan benar.`,
    });
  }
...
export const processMediaMessage = async (
  ...
  try {
    ...
    const fileName =
      mediaMessage.fileName || mediaMessage.title || `file-${Date.now()}`;
...
export async function generateSummaryAndQr(chatId: string, session: UserState) {
  const sock = getClient();
  await sock.sendMessage(chatId, {
  ...
  try {
    ...
    console.error(`[Worker] Error creating print job:`);
    console.log(error);
    await sock.sendMessage(chatId, {
